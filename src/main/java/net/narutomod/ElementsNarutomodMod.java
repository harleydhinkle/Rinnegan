...
public class ElementsNarutomodMod implements IFuelHandler, IWorldGenerator {
    ...
    public ElementsNarutomodMod() {
        ...
        sounds.put(new ResourceLocation("narutomod", "shot"), new net.minecraft.util.SoundEvent(new ResourceLocation("narutomod", "shot")));
        sounds.put(new ResourceLocation("narutomod", "gyuki_roar"),
                new net.minecraft.util.SoundEvent(new ResourceLocation("narutomod", "gyuki_roar")));
        sounds.put(new ResourceLocation("narutomod", "gyuki_snort"),
        ...
        sounds.put(new ResourceLocation("narutomod", "toadchant"), new net.minecraft.util.SoundEvent(new ResourceLocation("narutomod", "toadchant")));
        sounds.put(new ResourceLocation("narutomod", "hidan"), new net.minecraft.util.SoundEvent(new ResourceLocation("narutomod", "hidan")));
        sounds.put(new ResourceLocation("narutomod", "groundshock"),
    ...
    // -------------------------
    // UPDATED preInit (fixes dedicated server crash from client-only Render classes)
    // -------------------------
    public void preInit(FMLPreInitializationEvent event) {
        final boolean isClient = event.getSide().isClient();

        try {
            for (ASMDataTable.ASMData asmData : event.getAsmData().getAll(ModElement.Tag.class.getName())) {
                String className = asmData.getClassName();

                // Print BEFORE loading so you can see what element causes trouble
                System.out.println("[NarutoMod] Loading ModElement: " + className);

                try {
                    Class<?> clazz = Class.forName(className);
                    if (clazz.getSuperclass() == ElementsNarutomodMod.ModElement.class) {
                        elements.add((ElementsNarutomodMod.ModElement)
                                clazz.getConstructor(this.getClass()).newInstance(this));
                    }
                } catch (NoClassDefFoundError err) {
                    String missing = err.getMessage();

                    // On a dedicated server, skip mod-elements that depend on net.minecraft.client.*
                    if (!isClient && missing != null && missing.startsWith("net/minecraft/client/")) {
                        System.err.println("[NarutoMod] Skipping CLIENT-only element on server: " + className
                                + " (missing: " + missing + ")");
                        continue;
                    }

                    // If it's not client-only, rethrow so you still see real problems
                    throw err;
                }
            }
        } catch (Exception e) {
        ...
        this.addNetworkMessage(
                NarutomodModVariables.WorldSavedDataSyncMessageHandler.class,
                NarutomodModVariables.WorldSavedDataSyncMessage.class,
    ...
    public <T extends IMessage, V extends IMessage> void addNetworkMessage(Class<? extends IMessageHandler<T, V>> handler,
                                                                           Class<T> messageClass, Side... sides) {
        for (Side side : sides)
