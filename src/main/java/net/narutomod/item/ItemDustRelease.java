...
public class ItemDustRelease extends ElementsNarutomodMod.ModElement {
	@GameRegistry.ObjectHolder("narutomod:dust_release")
	...
	private static final int COOLDOWN_JINTON = 6000;
	...
	public ItemDustRelease(ElementsNarutomodMod instance) {
		super(instance, 367);
	}

	@Override
	public void initElements() {
		elements.items.add(() -> new ItemJinton.RangedItem(BEAM, CUBE));
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(block, 0, new ModelResourceLocation("narutomod:dust_release", "inventory"));
	}
	...
	public static class RangedItem extends ItemJutsu.Base {
		public RangedItem(ItemJutsu.JutsuEnum... list) {
			...
			this.setUnlocalizedName("dust_release");
			this.setRegistryName("dust_release");
			this.setCreativeTab(TabModTab.tab);
			this.defaultCooldownMap[BEAM.index] = COOLDOWN_JINTON;
			this.defaultCooldownMap[CUBE.index] = COOLDOWN_JINTON;
		}
			...
			if (!world.isRemote) {
				...
				if (power >= 1f && this.executeJutsu(itemstack, entity, power)
				 && entity instanceof EntityPlayer && !((EntityPlayer)entity).isCreative()) {
					...
					player.getCooldownTracker().setCooldown(itemstack.getItem(), COOLDOWN_JINTON);
				}
			}
		}
		...
		public ActionResult<ItemStack> onItemRightClick(World world, EntityPlayer entity, EnumHand hand) {
			if (entity.isCreative() || (ProcedureUtils.hasItemInInventory(entity, ItemFireRelease.block)
			 && ProcedureUtils.hasItemInInventory(entity, ItemWindRelease.block)
		...
		public void addInformation(ItemStack itemstack, World world, List<String> list, ITooltipFlag flag) {
			...
			list.add(TextFormatting.GREEN + net.minecraft.util.text.translation.I18n.translateToLocal("tooltip.dust_release.musthave") + TextFormatting.RESET);
		}
	}
	...
	public static class EntityBeam extends EntityBeamBase.Base implements ItemJutsu.IJutsu {
		...
		// ===== FIX (OPTION A): TOTAL CAP PER BEAM ENTITY (OVERALL, NOT PER TARGET) =====
		...
		protected void updatePosition() {
			if (this.shootingEntity != null) {
				Vec3d vec = this.shootingEntity.getPositionEyes(1f).subtract(0, 0.2f, 0)
					.add(this.shootingEntity.getLookVec().scale(0.5d));
				this.setPosition(vec.x, vec.y, vec.z);
			}
		}

		// ===== NEW: APPLY MULT + PER-HIT CAP + TOTAL PER-BEAM CAP =====
		private void attackEntityWithJutsuCappedTotal(Entity attacker, Entity target, float desiredAmount) {
			...
			float amount = (float)(desiredAmount * mult);
			if (amount <= 0.0f) {
				return;
			}
			...
			// per-hit cap is enforced inside attackEntityWithJutsu()
			attackEntityWithJutsu(this, attacker, target, amount);

			// track total used up by this beam
		...
		public class AirPunch extends ProcedureAirPunch {
			...
			protected void attackEntityFrom(Entity player, Entity target) {
				...
				float power = MathHelper.clamp(EntityBeam.this.getScale(), 0.5f, 10f);

				float desired = target instanceof EntityLivingBase
					? ((EntityLivingBase)target).getMaxHealth() * (float)d * 0.03f * power
					: Float.MAX_VALUE;

				// FIX: total cap is per BEAM entity lifetime (Option A)
				EntityBeam.this.attackEntityWithJutsuCappedTotal(player, target, desired);
			}
	...
	// Common damage function (used by BEAM and CUBE)
	private static void attackEntityWithJutsu(Entity projectile, Entity attacker, Entity target, float amount) {
		...
		if (target instanceof EntityLivingBase) {
			...
			target.attackEntityFrom(
				ItemJutsu.causeJutsuDamage(projectile, attacker).setDamageBypassesArmor().setDamageIsAbsolute(),
				amount
			);
		} else if (!target.attackEntityFrom(
			ItemJutsu.causeJutsuDamage(projectile, attacker).setDamageBypassesArmor().setDamageIsAbsolute(),
			amount
		)) {
	...
	public static class EntityCube extends EntityScalableProjectile.Base implements ItemJutsu.IJutsu {
		...
		private void destroyBlocksAndEntitiesInAABB(AxisAlignedBB bb) {
			...
			for (Entity entity : this.world.getEntitiesWithinAABBExcludingEntity(this, bb)) {
				...
				float dmg = entity instanceof EntityLivingBase
					? ((EntityLivingBase)entity).getMaxHealth() * (float)d * 0.25f
					: Float.MAX_VALUE;
				attackEntityWithJutsu(this, this.shootingEntity, entity, dmg);
			}
		}
				...
				if (this.ticksAlive <= this.wait) {
					...
					if (this.ticksAlive <= this.wait + this.growTime) {
						...
						Particles.spawnParticle(this.world, Particles.Types.FALLING_DUST, this.posX, this.posY + 0.25d * this.fullScale, this.posZ,
							(int)(this.fullScale * 6), this.fullScale * 0.1F, this.fullScale * 0.1F, this.fullScale * 0.1F,
							0D, 0D, 0D, 0xC0A0A0A0);
					}
				}
			}
			...
			public void onGetCollisionBoxes(GetCollisionBoxesEvent event) {
				for (EntityCube ec : event.getWorld().getEntitiesWithinAABB(EntityCube.class, event.getAabb().grow(10.0D))) {
					if (ec != event.getEntity() && ec.getEntityBoundingBox().intersects(event.getAabb())
						&& ec.getTicksAlive() >= ec.wait + ec.growTime
						&& ec.getTicksAlive() - ec.wait - ec.growTime <= ec.idleTime) {
