...
public class ItemYinRelease extends ElementsNarutomodMod.ModElement {
	@GameRegistry.ObjectHolder("narutomod:yin_release")
	...
	public ItemYinRelease(ElementsNarutomodMod instance) {
		super(instance, 441);
	}

	@Override
	public void initElements() {
		elements.items.add(() -> new ItemInton.RangedItem(GENJUTSU, MBTRANSFER, SHADOW_IMITATION));
	}

	@Override
	@SideOnly(Side.CLIENT)
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(block, 0, new ModelResourceLocation("narutomod:yin_release", "inventory"));
	}

	public static class RangedItem extends ItemJutsu.Base {
		public RangedItem(ItemJutsu.JutsuEnum... list) {
			super(ItemJutsu.JutsuEnum.Type.INTON, list);
			this.setUnlocalizedName("yin_release");
			this.setRegistryName("yin_release");
	...
	public static class Genjutsu implements ItemJutsu.IJutsuCallback {
		...
		private final int cooldown = 4800;
		...
		public static boolean canTargetBeAffected(EntityLivingBase caster, EntityLivingBase target) {
			if (target instanceof EntityTailedBeast.Base && !ItemSharingan.wearingAny(caster)) {
				...
				ItemStack stack = ProcedureUtils.getMatchingItemStack(target, ItemNinjaArts.block);
				if (stack != null && ItemNinjaArts.isJutsuEnabled(stack, ItemNinjaArts.BUGSWARM)) {
					return false;
				}
			}
			return true;
