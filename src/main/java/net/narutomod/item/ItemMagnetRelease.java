...
public class ItemMagnetRelease extends ElementsNarutomodMod.ModElement {
    @GameRegistry.ObjectHolder("narutomod:magnet_release")
    ...
    public void registerModels(ModelRegistryEvent event) {
        ModelLoader.setCustomModelResourceLocation(block, 0, new ModelResourceLocation("narutomod:magnet_release", "inventory"));
    }
    ...
    public static class RangedItem extends ItemJutsu.Base {
        public RangedItem(ItemJutsu.JutsuEnum... list) {
            ...
            this.setUnlocalizedName("magnet_release");
            this.setRegistryName("magnet_release");
        ...
        public ActionResult<ItemStack> onItemRightClick(World world, EntityPlayer entity, EnumHand hand) {
            if ((entity.isCreative() || (ProcedureUtils.hasItemInInventory(entity, ItemWindRelease.block)
                    && ProcedureUtils.hasItemInInventory(entity, ItemEarthRelease.block)))
        ...
        public void addInformation(ItemStack itemstack, World world, List<String> list, ITooltipFlag flag) {
            ...
            list.add(TextFormatting.GREEN + net.minecraft.util.text.translation.I18n.translateToLocal("tooltip.magnet_release.musthave") + TextFormatting.RESET);
        }
    }
    ...
    public static class EntitySandShield extends EntityShieldBase {
        ...
        private static final int MAX_PARTICLES_PER_SWARM = 50;

        // NEW: ONLY for SandShield (won't affect other jutsu)
        private static final int SHIELD_PARTICLE_LIFE = 120;   // 6 seconds
        private static final int SHIELD_SWARM_LIFETIME = 80;   // 4 seconds
        ...
        // clamped and swarm-count limited
        private void moveSand(Vec3d from, Vec3d to, int count) {
            if (this.world.isRemote) {
                return;
            }

            int clampedCount = Math.min(count, MAX_PARTICLES_PER_SWARM);

            if (!this.sandTargets.isEmpty() && this.sandTargets.size() >= MAX_SWARMS) {
                // reuse oldest swarm instead of creating a new one
                SwarmTarget st = this.sandTargets.get(0);
                st.setStartVec(from);
                st.setTarget(to, 0.6f, 0.02f, true);
                st.resetSpawned();
            } else {
                // IMPORTANT: lifetime caps only applied here (SandShield only)
                SwarmTarget st = new SwarmTarget(this.world, clampedCount, from, to, 0.95f, this.color)
                        .setLifetimes(SHIELD_PARTICLE_LIFE, SHIELD_SWARM_LIFETIME);
                this.sandTargets.add(st);
            }
        }
    ...
    public static class SandParticle extends EntityParticle.Base implements ISwarmEntity {
        ...
        // fewer particles & only every 2 ticks client-side
        @Override
        public void onUpdate() {
            super.onUpdate();
    ...
    public static class SwarmTarget<T extends Entity & ISwarmEntity> {
        ...
        // DEFAULTS (do not break other jutsu)
        private int particleLifeTicks = 300;
        private int swarmLifetimeTicks = -1; // -1 = unlimited
        ...
        public SwarmTarget(World worldIn, int totalIn, AxisAlignedBB startBBIn, AxisAlignedBB targetBBIn, Vec3d initialMotion, float speedIn, float inaccuracyIn, boolean dieOnReached, float scaleIn, int colorIn) {
            ...
            this.border = this.particles.isEmpty() ? this.startBB : this.particles.get(0).getEntityBoundingBox();
        }

        // apply caps only when caller wants them
        public SwarmTarget<T> setLifetimes(int particleLifeTicksIn, int swarmLifetimeTicksIn) {
            this.particleLifeTicks = Math.max(20, particleLifeTicksIn);
            this.swarmLifetimeTicks = swarmLifetimeTicksIn; // -1 = unlimited
            return this;
        }
        ...
        private void spawnNewParticles() {
            for (int i = 0; this.spawned < this.total && i < 1; i++, this.spawned++) {
                Vec3d vec = this.randomPosInBB(this.startBB);
                T p = this.createParticle(vec.x, vec.y, vec.z,
                        (rand.nextDouble() - 0.5d) * 2d * this.spawnMotion.x, this.spawnMotion.y,
                        (rand.nextDouble() - 0.5d) * 2d * this.spawnMotion.z,
                        this.color,
                        this.scale + (rand.nextFloat() - 0.5f) * this.scale * 0.2f,
                        this.particleLifeTicks);
        ...
        // REQUIRED by EntitySandBullet
        public boolean allParticlesReachedTarget() {
            if (this.border == null || this.targetBB == null) return false;
        ...
        protected void playFlyingSound(double x, double y, double z, float volume, float pitch) {
            this.world.playSound(null, x, y, z, SoundEvents.BLOCK_SAND_PLACE, SoundCategory.BLOCKS, volume, pitch);
        }

        public void onUpdate() {
            // optional swarm lifetime cap
            if (this.swarmLifetimeTicks >= 0 && ++this.ticks > this.swarmLifetimeTicks) {
                this.forceRemove();
                return;
            }
            ...
            if (!this.particles.isEmpty()) {
                ...
                if (!this.world.isRemote && this.ticks % 5 == 0) {
                    this.playFlyingSound(ep.posX, ep.posY, ep.posZ,
                            this.particles.size() * this.speed * 0.0025f,
                ...
                while (iter.hasNext()) {
                    ...
                    if (ep.isEntityAlive()) {
                        ...
                        if (this.dieOnTargetReached && vec.lengthVector() < 0.1d + this.inaccuracy) {
                            ...
                            ep.motionX += vec.x + rand.nextGaussian() * this.inaccuracy;
                            ep.motionY += vec.y + rand.nextGaussian() * this.inaccuracy;
                            ep.motionZ += vec.z + rand.nextGaussian() * this.inaccuracy;
        ...
        // REQUIRED by EntitySandLevitation
        public void setTarget(Vec3d newTarget, boolean dieOnReached) {
            this.setTarget(convert2AABB(newTarget, this.inaccuracy), dieOnReached);
        }
        ...
        public void setTarget(Vec3d newTargetPos, float speedIn, float inaccuracyIn, boolean dieOnReached) {
            this.setTarget(convert2AABB(newTargetPos, inaccuracyIn), speedIn, inaccuracyIn, dieOnReached);
        }
        ...
        private static AxisAlignedBB convert2AABB(Vec3d vec, float width) {
            return new AxisAlignedBB(vec.x - width * 0.5f, vec.y - width * 0.5f, vec.z - width * 0.5f,
                    vec.x + width * 0.5f, vec.y + width * 0.5f, vec.z + width * 0.5f);
        }
