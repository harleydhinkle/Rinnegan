...
public class ItemJiton extends ElementsNarutomodMod.ModElement {
	...
	public static class EntitySandShield extends EntityShieldBase {
		...
		private static final int MAX_PARTICLES_PER_SWARM = 80;
		...
		private void moveSand(Vec3d from, Vec3d to, int count) {
			...
			if (!this.sandTargets.isEmpty() && this.sandTargets.size() >= MAX_SWARMS) {
				...
				this.sandTargets.add(new SwarmTarget(this.world, clampedCount, from, to, 0.95f, this.color));
			}
		}
	...
	public static class SwarmTarget<T extends Entity & ISwarmEntity> {
		...
		// NEW: hard lifetime cap for the whole swarm
		private static final int SWARM_LIFETIME = 80; // 4 seconds at 20 TPS
		...
		// UPDATED: now uses the life parameter
		...
		private void spawnNewParticles() {
			...
			for (int i = 0; this.spawned < this.total && i < 1; i++, this.spawned++) {
				...
				T p = this.createParticle(vec.x, vec.y, vec.z,
				 ...
				 this.scale + (this.rand.nextFloat() - 0.5f) * this.scale * 0.2f, 120);
		...
		public void onUpdate() {
			// NEW: hard lifetime cap for the swarm
			if (++this.ticks > SWARM_LIFETIME) {
				this.forceRemove();
				return;
			}
			...
			if (!this.particles.isEmpty()) {
				...
				// sound throttled: only every 5 ticks server-side
