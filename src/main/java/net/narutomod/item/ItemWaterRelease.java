...
public class ItemWaterRelease extends ElementsNarutomodMod.ModElement {
	@GameRegistry.ObjectHolder("narutomod:water_release")
	...
	public ItemWaterRelease(ElementsNarutomodMod instance) {
		super(instance, 368);
	}

	@Override
	public void initElements() {
		elements.items.add(() -> new ItemSuiton.RangedItem(HIDINGINMIST, WATERBULLET, WATERDRAGON, WATERPRISON, WATERSHARK, WATERSHOCK, WATERBOMB, ACIDSPIT));
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(block, 0, new ModelResourceLocation("narutomod:water_release", "inventory"));
	}

	public static class RangedItem extends ItemJutsu.Base {
		public RangedItem(ItemJutsu.JutsuEnum... list) {
			super(ItemJutsu.JutsuEnum.Type.SUITON, list);
			this.setRegistryName("water_release");
			this.setUnlocalizedName("water_release");
			...
			if (!this.world.isRemote) {
				for (EntityLivingBase entity : this.world.getEntitiesWithinAABB(EntityLivingBase.class, this.getEntityBoundingBox().grow(this.radius+100))) {
					...
					if (entity instanceof EntityPlayer) {
						...
						aInstance.applyModifier(new AttributeModifier(FOLLOW_MODIFIER, "water_release.followModifier", -d0, 0));
					}
					if (!entity.equals(this.user) && this.user instanceof EntityPlayerMP) {
