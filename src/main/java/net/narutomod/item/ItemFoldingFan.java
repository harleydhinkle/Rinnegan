...
public class ItemFoldingFan extends ElementsNarutomodMod.ModElement {
	...
	public static class RangedItem extends Item implements ItemOnBody.Interface {
		...
		/**
		 * Charge ? release to fire Wind Blade.
		 */
		@Override
		public void onPlayerStoppedUsing(ItemStack itemstack, World world, EntityLivingBase entity, int timeLeft) {
			if (!world.isRemote) {
				// Remove fire like before
				entity.extinguish();

				// How long the player held right-click (in ticks)
				float useTime = (float)this.getMaxItemUseDuration(itemstack) - timeLeft;
				...
				// Cast Wind Blade
				new EntityWindBlade.EC.Jutsu().createJutsu(itemstack, entity, power);
