...
public class PlayerTracker extends ElementsNarutomodMod.ModElement {
    ...
    public static class PlayerHook {
        ...
        // keep your DAMAGE modifier
        private static final UUID DAMAGE_UUID = UUID.fromString("d7a3d8b0-1a1f-4b3a-9a8c-1f2e3f4d5e6f");
        ...
        public void onTick(TickEvent.PlayerTickEvent event) {
            if (event.phase == TickEvent.Phase.END && event.player instanceof EntityPlayerMP) {
                ...
                // === keep your DAMAGE scaling ===
                double damageBonus = getBattleXp(event.player) * 5.0E-4d;
                IAttributeInstance attackDamageAttr = event.player.getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE);
                if (attackDamageAttr != null) {
                    AttributeModifier damageMod = attackDamageAttr.getModifier(DAMAGE_UUID);
                    if (damageMod == null || damageMod.getAmount() != damageBonus) {
                        if (damageMod != null) {
                            attackDamageAttr.removeModifier(DAMAGE_UUID);
                        }
                        attackDamageAttr.applyModifier(new AttributeModifier(DAMAGE_UUID, "ninja.attackdamage", damageBonus, 0));
                    }
                }
        ...
        public void onDamaged(LivingDamageEvent event) {
            ...
            if (!targetEntity.equals(sourceEntity) && sourceEntity instanceof EntityLivingBase && amount > 0f) {
                ...
                // === base-jar style XP for the PLAYER GETTING HIT ===
                if (this.isOffCooldown(targetEntity) && targetEntity instanceof EntityPlayer && amount < ((EntityPlayer) targetEntity).getHealth()) {
                    double xpBase = getBattleXp((EntityPlayer) targetEntity);
                    double gained;

                    if (xpBase > (double)1.0F) {
                        gained = Math.min((double)amount / (double)MathHelper.sqrt(MathHelper.sqrt(xpBase)), (double)50.0F);
                    } else {
                        gained = 1d;
                    }

                    gained *= ModConfig.NINJAXP_MULTIPLIER;

                    if (gained > (double)0.0F) {
                        logBattleExp((EntityPlayer) targetEntity, gained);
                    }
                }

                // === your existing XP for the ATTACKER (kept) ===
                if (sourceEntity instanceof EntityPlayer) {
                    ...
                    if ((targetEntity instanceof EntityPlayer || (targetEntity instanceof EntityLiving && !((EntityLiving) targetEntity).isAIDisabled()))
                        ...
                        double x = MathHelper.sqrt(target.getMaxHealth() * ProcedureUtils.getModifiedAttackDamage(target)
                                * MathHelper.sqrt(ProcedureUtils.getArmorValue(target) +  (double)1.0F) * Math.min(resistance, 6));
                        xp = Math.min(x * Math.min(amount / target.getMaxHealth(), 1f), 50d);
        ...
        public void onWorldLoad(WorldEvent.Load event) {
            ...
            // === base jar: add this gamerule too ===
