...
public class EntityEarthSpears extends ElementsNarutomodMod.ModElement {
	...
	public void initElements() {
		elements.entities.add(() -> EntityEntryBuilder.create().entity(EC.class)
		 ...
		 .name("earth_spears").tracker(64, 3, true).build());
	}
	...
	public static class EC extends EntitySpike.Base implements ItemJutsu.IJutsu {
		...
		public EC(World worldIn) {
			...
			// no physical collision with blocks/entities
			this.noClip = true;
		}
		...
		// ---- disable collision / pushing, but keep AABB for damage scan ----
		@Override
		...
		public void applyEntityCollision(Entity entityIn) {
			// do nothing to avoid pushing entities around
		}

		@Override
		public net.minecraft.util.math.AxisAlignedBB getCollisionBoundingBox() {
			// returning null means it doesn't act as a solid obstacle
			return null;
		}
		// -------------------------------------------------------------------
		...
		public void onUpdate() {
			...
			if (this.ticksAlive <= this.growTime) {
				...
				if (!this.world.isRemote && this.world instanceof WorldServer) {
					...
					for (EntityLivingBase entity :
						this.world.getEntitiesWithinAABB(EntityLivingBase.class,
							this.getEntityBoundingBox().grow(1d, 0d, 1d))) {
				...
				// scale update (safe to do server-side only; synced via data manager)
				this.setEntityScale(MathHelper.clamp(
					this.maxScale * (float)this.ticksAlive / this.growTime,
					0.0f, this.maxScale
				));
			}
		}
			...
			public boolean createJutsu(ItemStack stack, EntityLivingBase entity, float power) {
				...
				if (res != null && res.typeOfHit == RayTraceResult.Type.BLOCK && res.sideHit == EnumFacing.UP) {
					...
					for (int i = 0; i < Math.round(power); i++) {
						...
						Vec3d vec = res.hitVec.addVector(
