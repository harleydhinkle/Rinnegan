...
public class EntityFalseDarkness extends ElementsNarutomodMod.ModElement {
    ...
    public void initElements() {
        elements.entities.add(() -> EntityEntryBuilder.create().entity(EC.class)
                .id(new ResourceLocation("narutomod", "false_darkness"), ENTITYID)
                .name("false_darkness")
                .tracker(64, 3, true)
                .build());
    }
    ...
    public static class EC extends Entity implements ItemJutsu.IJutsu {
        ...
        public void onUpdate() {
            ...
            // Prevent divide-by-zero / unstable charge
            int buildtime = Math.max(1, (int)(this.power * 20f));

            // === CHARGE PHASE ===
            if (this.ticksExisted <= buildtime) {
                float f = Math.min((float)this.ticksExisted / (float)buildtime, 1.0f);

                if (this.rand.nextFloat() <= f * 0.2f) {
                    this.playSound(
                            SoundEvent.REGISTRY.getObject(new ResourceLocation("narutomod:electricity")),
                            0.4f,
                            this.rand.nextFloat() * 0.5f + 1.5f
                    );
                }

                for (int i = 0; i < (int)(f * 8f); i++) {
                    EntityLightningArc.spawnAsParticle(
                            this.world,
            ...
            // === STRIKE PHASE (SERVER ONLY) ===
            if (!this.world.isRemote) {
                if (this.target == null || this.target.isDead || this.target.world != this.world) {
                    this.setDead();
                    return;
                }
                ...
                EntityLightningArc.Base arc = new EntityLightningArc.Base(
                        this.world,
                        this.getPositionVector(),
                        this.target.getPositionEyes(1f),
                        0x000000FF,
                        40,
                        0f
                );
                ...
                arc.setDamage(
                        ItemJutsu.causeJutsuDamage(this, this.user),
                        BASE_DAMAGE * this.power,
                        this.user
                );
        ...
        public static class Jutsu implements ItemJutsu.IJutsuCallback {
            ...
            public boolean createJutsu(ItemStack stack, EntityLivingBase entity, float power) {
                // Server controls spawn (prevents ghost casts)
