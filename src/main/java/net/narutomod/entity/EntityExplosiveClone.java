...
import net.minecraft.util.text.TextComponentString; // <-- added
...
public class EntityExplosiveClone extends ElementsNarutomodMod.ModElement {
	...
	public static class EC extends EntityClone.Base implements ItemJutsu.IJutsu {
		...
		private static final int MAX_CLONES = 20; // <-- max per summoner
		...
		public static class Jutsu implements ItemJutsu.IJutsuCallback {
			...
			public boolean createJutsu(ItemStack stack, EntityLivingBase entity, float power) {
				if (!entity.isSneaking()) {
					...
					if (count >= EC.MAX_CLONES) {
						if (entity instanceof EntityPlayer && !entity.world.isRemote) {
							((EntityPlayer)entity).sendStatusMessage(
							 new TextComponentString("You already have " + EC.MAX_CLONES + " explosive clones!"), true);
						}
						return false;
					}
					// ---------------------------------------
