...
import net.narutomod.Chakra; // ? Added import for chakra usage
	...
	public static class EC extends Entity implements ItemJutsu.IJutsu {
		...
		public void onUpdate() {
			...
			if (this.user != null && this.user.isEntityAlive()) {
				...
				// ? Chakra Maintenance Drain System
				if (!this.world.isRemote) {
					double chakraCostPerTick = 2.0d; // ~40 chakra/second, adjust for balance
					if (!Chakra.pathway(this.user).consume(chakraCostPerTick)) {
						this.user.sendMessage(new net.minecraft.util.text.TextComponentString("You're out of chakra! The jutsu collapses."));
						this.shouldDie = true;
						return;
					}
				}
