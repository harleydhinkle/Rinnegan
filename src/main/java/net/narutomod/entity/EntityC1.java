    ...
    public static class EC extends ItemExplosionRelease.ExplosiveClay {
        ...
        public boolean attackEntityAsMob(Entity entityIn) {
            if (super.attackEntityAsMob(entityIn)) {
                ...
                DamageSource src = ItemJutsu.causeJutsuDamage(this, this.getOwner());
                return entityIn.attackEntityFrom(src, 8f + this.rand.nextFloat() * 4f);
            }
            return false;
        }
    }

    // CLIENT ONLY: safe because FML strips this method on server (SideOnly)
    @SideOnly(Side.CLIENT)
    @Override
    public void preInit(FMLPreInitializationEvent event) {
        EntityC1Renderer.register();
    }
}

/* =======================================================================================
   Everything below is CLIENT-ONLY, but kept in the SAME .java file to match your request.
   IMPORTANT: These are TOP-LEVEL classes (not inner classes of EntityC1),
              so Sponge won't try to load EntityC1$Renderer on the server.
   ======================================================================================= */

@SideOnly(Side.CLIENT)
class EntityC1Renderer {
    private static final ResourceLocation TEXTURE = new ResourceLocation("narutomod:textures/vex1.png");

    static void register() {
        RenderingRegistry.registerEntityRenderingHandler(EntityC1.EC.class, renderManager -> new RenderCustom(renderManager));
    }

    private static class RenderCustom extends RenderBiped<EntityC1.EC> {
        RenderCustom(RenderManager renderManagerIn) {
            super(renderManagerIn, new EntityC1ModelCustom(), 0.3F);
        }

        @Override
        protected void preRenderCallback(EntityC1.EC entity, float partialTickTime) {
            GlStateManager.scale(0.4F, 0.4F, 0.4F);
        }

        @Override
        protected ResourceLocation getEntityTexture(EntityC1.EC entity) {
            return TEXTURE;
        }
    }
}
...
@SideOnly(Side.CLIENT)
class EntityC1ModelCustom extends ModelBiped {
    ...
    EntityC1ModelCustom(float modelSize) {
