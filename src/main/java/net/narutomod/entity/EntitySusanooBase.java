...
import net.narutomod.item.ItemAnbuSword;
...
public abstract class EntitySusanooBase extends EntityCreature implements IRangedAttackMob, EntitySummonAnimal.ISummon {
	...
	// === SUSANOO COOLDOWN (PER-PLAYER) ===
	private static final String NBT_COOLDOWN_KEY = "susanooCooldown";
	...
	public EntitySusanooBase(EntityLivingBase player) {
		...
		// NOTE: cooldown is NOT checked here anymore, so form-switching that
		// spawns a new Susanoo entity won't be blocked by this base class.
		...
		//     .setBaseValue(ProcedureUtils.getModifiedSpeed(player) * 0.3d);
	...
	/**
	 * Original API for other Naruto classes.
	 */
	@Nullable
	...
	public int getFlameColor() {
		return this.dataManager.get(FLAME_COLOR);
	}

	/**
	 * ISummon API used by ShinobiAddon & other stuff.
	 */
	@Override
	@Nullable
	public EntityLivingBase getSummoner() {
		// Just reuse existing owner logic
	...
	public boolean attackEntityFrom(DamageSource source, float amount) {
		...
		if (flag && summoner != null && !this.isEntityAlive()) {
			summoner.attackEntityFrom(source,
				CombatRules.getDamageAfterAbsorb(amount, (float)this.getTotalArmorValue(), 0f) - f);
		}
		return flag;
	}
	...
	public boolean attackEntityAsMob(Entity entityIn) {
		...
		if (passenger instanceof EntityLivingBase) {
			...
			if (flag && entityIn instanceof EntityLivingBase) {
				...
				if (f2 > 0.8f) {
					ProcedureUtils.pushEntity(this, entityIn,
						ProcedureUtils.getReachDistance(this) + 1.0d, f2 * 2.5F);
					this.motionX *= 0.6D;
					this.motionZ *= 0.6D;
				}
			}
	...
	// Clean addPassenger: no reflection, use vanilla handling
	@Override
	...
	public void onLivingUpdate() {
		...
		if (!this.world.isRemote) {
			// Try to recover owner from rider if missing
			if (ownerPlayer == null) {
				Entity rider = this.getControllingPassenger();
				if (rider instanceof EntityLivingBase) {
					this.setOwnerPlayer((EntityLivingBase) rider);
					ownerPlayer = (EntityLivingBase) rider;
					flag = ownerPlayer instanceof EntityPlayer;
				}
			}

			// Only kill if we actually have an owner and they're gone/disconnected
			if (ownerPlayer != null) {
				if (!ownerPlayer.isEntityAlive()
						|| (ownerPlayer instanceof EntityPlayerMP && ((EntityPlayerMP) ownerPlayer).hasDisconnected())) {
					this.setDead();
				}
			}
			// IMPORTANT: we do NOT kill just because ownerPlayer == null
		}

		if (flag && ownerPlayer != null) {
		...
		for (int i = 0; i < (int) this.height; i++) {
			...
			this.world.spawnAlwaysVisibleParticle(Particles.Types.FLAME.getID(), d0, d1, d2,
				0.0D, 0.05D, 0.0D, this.getFlameColor(), (int)(this.width * 15f));
		}
	}
	...
	// === START COOLDOWN WHEN SUSANOO IS REMOVED ===
	@Override
	public void setDead() {
		if (!this.world.isRemote) {
			EntityLivingBase owner = this.getSummoner();
			if (owner instanceof EntityPlayer) {
				...
				ep.getEntityData().setLong(NBT_COOLDOWN_KEY, now + COOLDOWN_TICKS);
			}
		}
		super.setDead();
	}
    ...
    protected void showHeldWeapons() {
		...
		if (!this.world.isRemote && owner != null) {
			...
			// Show sword for multiple weapon types (like the decompiled version)
			this.setShowSword(
				stack.getItem() == ItemChokuto.block ||
				stack.getItem() == Items.IRON_SWORD ||
				stack.getItem() == ItemKunai.block ||
				stack.getItem() == ItemAnbuSword.block
			);
	...
	public static class AIAttackRangedAndMoveTowardsTarget extends EntityAIBase {
		...
		public void updateTask() {
		    --this.navigateDelay;
		    double d0 = this.entityHost.getDistance(this.attackTarget.posX,
