...
public class EntityKirin extends ElementsNarutomodMod.ModElement {
	...
	public static class EC extends EntityScalableProjectile.Base implements ItemJutsu.IJutsu {
		...
		protected void onImpact(RayTraceResult result) {
			...
			if (!this.world.isRemote) {
				...
				// Impact position (entity or block)
				...
				// Existing AoE + fire
				ProcedureAoeCommand.set(this.world, vec.x, vec.y, vec.z, 0.0D, 10.0D)
				 .exclude(this)
				 .exclude(this.shootingEntity)
				 .setFire(15)
				 .damageEntities(ItemJutsu.causeJutsuDamage(this, this.shootingEntity), 100f * size);

				// EXTRA AoE damage around the impact
				double radius = 8.0D * size; // 80-block radius at scale 10
				AxisAlignedBB aabb = new AxisAlignedBB(
					vec.x - radius, vec.y - 2.0D, vec.z - radius,
					vec.x + radius, vec.y + 4.0D, vec.z + radius
				);
	...
	public static void startWeatherThunder(Entity entity, int ticks) {
		...
		entity.world.playSound(
			...
			1000.0F,
			0.8F + entity.world.rand.nextFloat() * 0.2F
		);
	}
