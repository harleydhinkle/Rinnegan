...
public class EntityWoodForest extends ElementsNarutomodMod.ModElement {
    ...
    try {
        // start from this segment's position (must use ints in 1.12)
        pos.setPos((int)Math.floor(this.posX), (int)Math.floor(this.posY), (int)Math.floor(this.posZ));

        int tries = 0;
        final int maxTries = 64; // safety cap so we don't loop forever
        ...
        while (!this.world.isAirBlock(pos) && tries++ < maxTries) {
            ...
            // If we wander out of the valid build height, bail out
            if (pos.getY() <= 0 || pos.getY() >= this.world.getHeight()) {
                break;
            }
        }

        // Only place leaves if we actually ended on air
        if (this.world.isAirBlock(pos)) {
