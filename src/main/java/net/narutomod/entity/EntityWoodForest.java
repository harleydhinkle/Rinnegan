    ...
    public void initElements() {
        elements.entities.add(() ->
                EntityEntryBuilder.create()
                        .entity(EC.class)
                        .id(new ResourceLocation("narutomod", "wood_forest"), ENTITYID)
                        .name("wood_forest")
                        .tracker(64, 3, true)
                        .build()
        );
    ...
    public static class EC extends ItemMokuton.WoodSegment implements ItemJutsu.IJutsu {
        ...
        public void onUpdate() {
            ...
            if (parent != null && this.ticksExisted < this.lifespan) {
                // ? FIXED: Allow client to render root segment (visible in Sponge multiplayer)
                if ((!this.world.isRemote || this.getIndex() == 0)) {
                    int index = this.getIndex();

                    // root segment
                    if (index == 0 && this.ticksExisted == 1) {
                        for (int i = 0; i < (int)(this.diameter * 0.6); i++) {
                            Vec3d vec = this.getPositionVector().add(
                                    new Vec3d(0, 1, this.rand.nextFloat() * this.diameter * 0.5f)
                                            .rotateYaw(this.rand.nextFloat() * (float)Math.PI * 2)
                            );
                            vec = new Vec3d(vec.x,
                                    (double)ProcedureUtils.getTopSolidBlockY(this.world, new BlockPos(vec)) - 0.5d,
                                    vec.z).subtract(this.getPositionVector());

                            EC segment = new EC(this, vec.x, vec.y, vec.z,
                                    (this.rand.nextFloat()-0.5f)*360f,
                                    this.rand.nextFloat()*30f);
                            segment.setLifespan(this.lifespan - this.ticksExisted * 2);
                            segment.lastSegment = segment;
                            segment.setSize(Math.max(this.diameter * 0.03f + 0.3f, 0.6f),
                                    Math.max(this.diameter * 0.03f + 0.3f, 0.6f));

                            this.world.spawnEntity(segment);
                        }

                        this.potentialTargets = this.world.getEntitiesWithinAABB(EntityLivingBase.class,
                                this.getEntityBoundingBox().grow(this.diameter, 0, this.diameter).expand(0, this.diameter, 0).expand(0, -5, 0),
                                (p) -> ItemJutsu.canTarget(p)
                                        && (!(p instanceof EntityPlayer) || !((EntityPlayer)p).isCreative())
                    ...
                    if (index > 0 && index < 15 && this.lastSegment != null && this.ticksExisted > 1) {
                        ...
                        if (this.ticksExisted <= 18 || (hasTarget && this.ticksExisted < 40)) {
                            if (!hasTarget && parent.rand.nextInt(8) == 7 && this.width > 0.32f) {
                                EC segment = new EC(this.lastSegment,
                                        (this.rand.nextFloat()-0.5f)*360f,
                                        (this.rand.nextFloat()-0.5f)*120f);
                            ...
                            this.lastSegment = new EC(this.lastSegment,
                                    (this.rand.nextFloat()-0.5f)*30f,
                                    (this.rand.nextFloat()-0.5f)*30f);
                    ...
                    // leaves
                    if (index > 16 && this.ticksExisted < 5 && !this.inGround && parent != null && parent.diameter > 25f && !this.hasLivingTarget()) {
                        BlockPos.PooledMutableBlockPos pos = BlockPos.PooledMutableBlockPos.retain();
                        try {
                            pos.setPos((int)Math.floor(this.posX), (int)Math.floor(this.posY), (int)Math.floor(this.posZ));
                            int tries = 0;
                            while (!this.world.isAirBlock(pos) && tries++ < 64) {
                                EnumFacing dir = EnumFacing.random(this.rand);
                                int distance = this.rand.nextInt(Math.max((int)this.width + 1, 2));
                                pos.move(dir, distance);
                                if (pos.getY() <= 0 || pos.getY() >= this.world.getHeight()) break;
                            }

                            if (this.world.isAirBlock(pos)) {
                                new net.narutomod.event.EventSetBlocks(
                                        this.world,
                                        ImmutableMap.of(pos.toImmutable(), Blocks.LEAVES.getStateFromMeta(0)),
                                        0,
        ...
        public static class Jutsu implements ItemJutsu.IJutsuCallback {
            ...
            public boolean createJutsu(ItemStack stack, EntityLivingBase entity, float power) {
                if (power >= 10.0f) {
                    ...
                    if (res != null && res.typeOfHit == RayTraceResult.Type.BLOCK) {
                        ...
                        ItemJutsu.setCurrentJutsuCooldown(stack, entity, COOLDOWN_TICKS);
                        return true;
                    }
                }
                return false;
            ...
            @Override
            public float getBasePower() { return 9.0f; }

            @Override
            public float getPowerupDelay() { return 30.0f; }
            ...
            public float getMaxPower(ItemStack stack, EntityLivingBase entity) {
                ...
                return Math.min(modifier * modifier * 19.49f, 25.0f);
            }
        }
    }
}
