...
public class EntityRasenshuriken extends ElementsNarutomodMod.ModElement {
    ...
    public static class EC extends EntityScalableProjectile.Base implements ItemJutsu.IJutsu {
        ...
        // Rasenshuriken-only cooldown (2 minutes = 2400 ticks)
        private static final int RASENSHURIKEN_COOLDOWN_TICKS = 2400;
        private static final String NBT_RASENSHURIKEN_CD = "RasenshurikenCooldown";

        private static boolean canUseRasenshuriken(ItemStack stack, EntityLivingBase entity) {
            if (stack == null || stack.isEmpty() || entity == null || entity.world == null) {
                return true;
            }
            ...
            long until = nbt.getLong(NBT_RASENSHURIKEN_CD);
            return entity.world.getTotalWorldTime() >= until;
        }

        private static long getRemainingCooldownTicks(ItemStack stack, EntityLivingBase entity) {
            if (stack == null || stack.isEmpty() || entity == null || entity.world == null) {
                return 0L;
            }
            NBTTagCompound nbt = stack.getTagCompound();
        ...
        private static void setRasenshurikenCooldown(ItemStack stack, EntityLivingBase entity) {
            if (stack == null || stack.isEmpty() || entity == null || entity.world == null) {
                return;
            }
            NBTTagCompound nbt = stack.getTagCompound();
            ...
            long until = entity.world.getTotalWorldTime() + RASENSHURIKEN_COOLDOWN_TICKS;
            nbt.setLong(NBT_RASENSHURIKEN_CD, until);
        }
        ...
        protected void doImpactDamage() {
            ProcedureAoeCommand.set(this.world, this.impactVec.x, this.impactVec.y, this.impactVec.z, 0d, this.width/2)
                    ...
                    .exclude(EntityTruthSeekerBall.EntityCustom.class)
                    .resetHurtResistanceTime()
            ...
            if (!this.world.isRemote) {
                List<EntityLivingBase> entities = this.world.getEntitiesWithinAABB(
                        ...
                        this.getEntityBoundingBox().grow(this.width / 2d)
                );

                for (EntityLivingBase target : entities) {
                    if (target.equals(this.shootingEntity)) continue;

                    net.narutomod.Chakra.Pathway cp = net.narutomod.Chakra.pathway(target);
                    if (cp != null) {
                        double victimChakra = cp.getAmount();
                        double maxChakra = cp.getMax();

                        if (maxChakra <= 0 || victimChakra <= 0) continue;

                        double percentDrain = Math.min(0.15d + (this.fullScale * 0.03d), 0.4d);
                        double drainAmount = maxChakra * percentDrain;

                        cp.consume(drainAmount);

                        if (target instanceof EntityPlayer) {
                            int shownDrain = (int) Math.min(drainAmount, victimChakra);
                            ((EntityPlayer)target).sendStatusMessage(
                                    new TextComponentString("-" + shownDrain + " Chakra"), true
                            );
                        }
                    }
                }
        ...
        public static class Jutsu implements ItemJutsu.IJutsuCallback {
            ...
            public boolean createJutsu(ItemStack stack, EntityLivingBase entity, float power) {
                // Rasenshuriken-only cooldown check
                if (!EC.canUseRasenshuriken(stack, entity)) {
                    if (entity instanceof EntityPlayer && !entity.world.isRemote) {
                        long remainingTicks = EC.getRemainingCooldownTicks(stack, entity);
                        long remainingSeconds = remainingTicks / 20L;
                        ((EntityPlayer)entity).sendStatusMessage(
                                new TextComponentString("Rasenshuriken is on cooldown (" + remainingSeconds + "s)"), true
                        );
                    }
                    return false;
                }
        ...
        public static class TSBVariant implements ItemJutsu.IJutsuCallback {
            ...
            @Override
            public boolean createJutsu(ItemStack stack, EntityLivingBase entity, float power) {
                // Rasenshuriken-only cooldown check (TSB variant)
                if (!EC.canUseRasenshuriken(stack, entity)) {
                    if (entity instanceof EntityPlayer && !entity.world.isRemote) {
                        long remainingTicks = EC.getRemainingCooldownTicks(stack, entity);
                        long remainingSeconds = remainingTicks / 20L;
                ...
                // Start Rasenshuriken cooldown for TSB variant too
                EC.setRasenshurikenCooldown(stack, entity);
