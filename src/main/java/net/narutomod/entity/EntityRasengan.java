...
public void applyEntityCollision(Entity entityIn) {
    if (this.ticksAlive > this.growTime && this.shootingEntity != null
     && !entityIn.equals(this.shootingEntity) && !this.bunshinHasSameSummoner(entityIn)) {
        ...
        float intendedDamage = 10f + this.fullScale * this.fullScale * 20f;

        // create a DamageSource that bypasses armor/enchantments
        DamageSource ds = this.damageSource;
        if (ds == null) {
            ds = ItemJutsu.causeJutsuDamage(this, this.shootingEntity);
        }
        ds = ds.setDamageBypassesArmor(); // bypass armor/protection

        float finalDamage = intendedDamage;

        // If the target is an EntityLivingBase, check for Resistance potion
        if (entityIn instanceof EntityLivingBase) {
            EntityLivingBase target = (EntityLivingBase) entityIn;
            if (target.isPotionActive(MobEffects.RESISTANCE)) {
                int amp = target.getActivePotionEffect(MobEffects.RESISTANCE).getAmplifier();
                // Resistance reduces damage by 20% per level (amplifier 0 = 20%, ...
                float reduction = 0.2f * (amp + 1);

                // if reduction >= 1.0f, the potion would normally make them invulnerable;
                // avoid division by zero or infinite multiplier by capping reduction slightly below 1.
                if (reduction >= 0.999f) {
                    reduction = 0.999f;
                }

                float multiplier = 1.0f / (1.0f - reduction);
                // safety: prevent absurdly huge multipliers
                if (multiplier > 20.0f) multiplier = 20.0f;

                finalDamage = intendedDamage * multiplier;
            }
        }

        if (entityIn.attackEntityFrom(ds, finalDamage)) {
