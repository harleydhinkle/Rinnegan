...
public class EntityNinjaMob extends ElementsNarutomodMod.ModElement {
    public static final List<Class<? extends Base>> TeamKonoha = Arrays.asList(
            EntityTenten.EntityCustom.class,
            EntitySakuraHaruno.EntityCustom.class,
            EntityIrukaSensei.EntityCustom.class,
            EntityMightGuy.EntityCustom.class
    );
    public static final List<Class<? extends Base>> TeamZabuza = Arrays.asList(
            EntityZabuzaMomochi.EntityCustom.class,
            EntityHaku.EntityCustom.class
    );
    public static final List<Class<? extends Base>> TeamPain = Arrays.asList(
            EntityPainDeva.EntityCustom.class,
            EntityPainAsura.EntityCustom.class,
            EntityPainAnimal.EntityCustom.class,
            EntityPainHuman.EntityCustom.class,
            EntityPainNaraka.EntityCustom.class,
            EntityPainPreta.EntityCustom.class,
            EntityNagato.EntityCustom.class
    );
    public static final List<Class<? extends Base>> TeamAkatsukiMembers = Arrays.asList(
            EntityItachi.EntityCustom.class,
            EntityKisameHoshigaki.EntityCustom.class,
            EntitySasori.EntityCustom.class,
            EntityDeidara.EntityCustom.class,
            EntityHidan.EntityCustom.class,
            EntityKakuzu.EntityCustom.class,
            EntityKonan.EntityCustom.class,
            EntityZetsu.EntityCustom.class,
            EntityObito.EntityTobi.class
    );
    public static final List<Class<? extends Base>> TeamAkatsuki =
            Stream.concat(TeamPain.stream(), TeamAkatsukiMembers.stream()).collect(Collectors.toList());
    ...
    public static abstract class Base extends EntityCreature {
        ...
        // ? CHANGED: private -> protected (so LayerInventoryItem + packet handler can access)
        protected final NonNullList<ItemStack> inventory = NonNullList.<ItemStack>withSize(inventorySize, ItemStack.EMPTY);
        ...
        protected void despawnEntity() {
            ...
            if (this.isNoDespawnRequired()) {
                this.idleTime = 0;
                    ...
                    && (result = net.minecraftforge.event.ForgeEventFactory.canEntityDespawn(this)) != net.minecraftforge.fml.common.eventhandler.Event.Result.DEFAULT) {
        ...
        protected boolean canSeeInvisible(Entity entityIn) {
            if (entityIn.isInvisible()) {
                // ? FIXED: distance to THIS, not distance to itself
                double d0 = this.getDistance(entityIn);
    ...
    public static class AIAttackMelee extends EntityAIBase {
            ...
            if (entitylivingbase == null) {
                ...
                if (this.path != null) {
                    ...
                    return this.getAttackReachSqr(entitylivingbase) >= this.attacker.getDistanceSq(
                            entitylivingbase.posX, entitylivingbase.getEntityBoundingBox().minY, entitylivingbase.posZ);
                }
            }
        }
        ...
        public void updateTask() {
            ...
            if ((this.longMemory || this.attacker.getEntitySenses().canSee(entitylivingbase)) && this.delayCounter <= 0
                ...
                if (d0 > this.getAttackReachSqr(entitylivingbase) * 0.8d
        ...
        public void onUpdateNavigation() {
            if (!this.noPath()) {
                ...
                if (this.targetPosition != null) {
                    ...
                    double d2 = this.entity.getDistanceSqToCenter(new BlockPos(
                    ...
                    if (d2 >= d0 && d1 <= this.entity.stepHeight && d1 >= -12d * this.entity.height) {
                        this.entity.getMoveHelper().setMoveTo(
                                (double) this.targetPosition.getX() + 0.5d,
                                (double) this.targetPosition.getY(),
                                (double) this.targetPosition.getZ() + 0.5d,
                                this.speed
                        );
                    } else {
                        this.targetPosition = null;
    ...
    public static class InventoryMessage implements IMessage {
        ...
        public InventoryMessage(Base entity) {
            this.id = entity.getEntityId();
            // ? CHANGED: copy list instead of holding reference
            this.list = Lists.newArrayList(entity.inventory);
        }
