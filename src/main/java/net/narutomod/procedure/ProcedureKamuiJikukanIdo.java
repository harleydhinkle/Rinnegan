...
public class ProcedureKamuiJikukanIdo extends ElementsNarutomodMod.ModElement {
    ...
    public static void executeProcedure(Map<String, Object> dependencies) {
        ...
        // =====================================================================
        // Gamerule guard: disable Kamui (and (optionally) Hiding in Rock) globally
        // This must be early so NOTHING in this procedure can activate Kamui.
        // =====================================================================
        if (!world.isRemote && PlayerTracker.noKamuiAndHidingInRock(world)) {
            // Reset any active Kamui state so players can't remain intangible/teleporting
            ...
            entity.getEntityData().setBoolean("kamui_intangible", false);
            entity.getEntityData().setDouble("kamui_timer", -1d);

            // hard stop physics flags
            entity.noClip = false;

            // make sure player abilities aren't stuck (integrated server = still server-side)
            if (entity instanceof EntityPlayer) {
                ...
                if (!p.world.isRemote) {
                    p.sendStatusMessage(new TextComponentString("Kamui is disabled by gamerule: noKamuiAndHidingInRock"), true);
                }
            }
            return;
        }
                        ...
                        if ((((timer) % 60) == 1)) {
                            world.playSound((EntityPlayer) null, x, y, z,
                                    (net.minecraft.util.SoundEvent) net.minecraft.util.SoundEvent.REGISTRY
                                            .getObject(new ResourceLocation("narutomod:kamui")),
                    ...
                    if ((f2)) {
                        ...
                        if (((!(f3)) && ((i) <= 0.99999))) {
                            if (((i) > 0)) {
                                ...
                                t.entityHit.attackEntityFrom(
                                        DamageSource.causeMobDamage((EntityLivingBase) entity).setDamageBypassesArmor().setDamageIsAbsolute(),
                                        Math.min((float) i, 1024f));
                            }
                        } else {
