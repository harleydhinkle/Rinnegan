...
public class ProcedureSusanoo extends ElementsNarutomodMod.ModElement {
	...
	// Same cooldown key as in EntitySusanooBase
	...
	public static void execute(EntityPlayer player) {
		...
		// === COOLDOWN CHECK ===
		// Only block NEW activation. If Susanoo is already active (susanoo_activated = true),
		// we still allow this procedure to run so it can DEACTIVATE it.
		if (!world.isRemote && !player.isCreative()) {
			NBTTagCompound data = player.getEntityData();
			long now = world.getTotalWorldTime();
			long cooldownEnd = data.getLong(NBT_COOLDOWN_KEY);

			if (cooldownEnd > now && !data.getBoolean("susanoo_activated")) {
				long remaining = cooldownEnd - now;
				long seconds = remaining / 20L;

				// Show remaining time to the player
				player.sendStatusMessage(
					new TextComponentString("Susanoo is on cooldown: " + seconds + "s remaining"),
					true
				);
				return; // Block new summon
			}
		}
		// === END COOLDOWN CHECK ===
