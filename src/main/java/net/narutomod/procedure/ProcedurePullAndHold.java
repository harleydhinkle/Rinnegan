...
public class ProcedurePullAndHold {
	...
	// ==== TIMER (NO MULTIPLYING, RAW TICKS ONLY) ====
	private int holdTicks = 0;
	private static final int MAX_HOLD_TICKS = 100; // example: 600 ticks
	// ===============================================
	...
	public void execute(boolean is_pressed, Entity puller, @Nullable Entity target) {
		...
		if (is_pressed) {
			...
			// Start grabbing if new target
			if (this.grabbedEntity == null) {
				this.grabbedEntity = target;
				this.holdTicks = 0; // reset tick counter when grabbing starts
			}

			// ---- TIMER CHECK (RAW TICK COUNTER) ----
			this.holdTicks++;
			...
			if (this.grabbedEntity instanceof net.minecraft.entity.item.EntityItem
			 || this.grabbedEntity instanceof net.minecraft.entity.item.EntityXPOrb) {
				...
				this.grabbedEntity.setPositionAndUpdate(
					vec3d.x,
					vec3d.y - this.grabbedEntity.height / 2.0F,
					vec3d.z
				);
	...
	public void reset() {
		...
		this.holdTicks = 0; // reset tick counter
	}
}
